> **Como usar este arquivo.** No Claude Code, salve como `CLAUDE.md` na raiz do projeto — as regras sobrevivem à compactação de contexto e a novas sessões. A nota acima da linha não faz parte do prompt operativo.
>
> _Versão enxuta v03. Igual à v02, com a adição do bloco `<comandos>`, que define os atalhos `/inicie` e `/encerre` apontando para as rotinas de início e encerramento já descritas em `<working_method>`. As instruções por extenso continuam sendo a fonte da verdade; os atalhos são só a porta de entrada rápida. Preserva o núcleo financeiro por inteiro e mantém as periferias reduzidas: sem assistente de importação (carga histórica manual na v1), sem OAuth/sincronização do Gmail (cobrança apenas assistida), sem PWA, backup simplificado. Todos acrescentáveis depois, com o núcleo já estável._
...
Use Git desde o início. Fluxo por etapa: leia os docs relevantes → verifique requisitos → atualize docs se necessário → implemente → atualize testes → registre no CHANGELOG.

Ritmo de sessão:
- Início: releia `CLAUDE.md`, `docs/ROADMAP.md` e o log do git, e diga ao usuário onde o projeto parou antes de propor o próximo passo.
- Durante: faça commits pequenos, claros e ligados a uma única etapa ao concluir cada etapa aprovada — não deixe para commitar só no fim do dia.
- Antes de qualquer push: confirme com o usuário que a branch é `main` e que nenhum segredo (`.env`, chaves, senhas, tokens) está sendo enviado.
- Encerramento (fim do dia ou quando o usuário pedir para parar): atualize `docs/ROADMAP.md` e `docs/CHANGELOG.md` com o que foi feito, faça commit e push, e confirme que o envio foi concluído antes de finalizar a sessão.
</working_method>

<comandos>
Estes atalhos são convenções combinadas com o usuário. Quando ele digitar um deles, execute a rotina correspondente já definida em <working_method>, sem exigir a instrução por extenso:

- `/inicie` — rotina de início de sessão: releia `CLAUDE.md`, `docs/ROADMAP.md` e o log do git, e diga ao usuário onde o projeto parou antes de propor o próximo passo.
- `/encerre` — rotina de encerramento: atualize `docs/ROADMAP.md` e `docs/CHANGELOG.md` com o que foi feito; confirme que a branch é `main` e que nenhum segredo está sendo enviado; faça commit e push; e confirme que o envio foi concluído antes de finalizar a sessão.

Se em alguma sessão você não reconhecer um atalho, o usuário pode dar a instrução por extenso — o conteúdo canônico está em <working_method>.
</comandos>
...
Nenhuma regra importante deve existir apenas na conversa ou apenas no código. Estes documentos são a memória do projeto: quando o contexto for comprimido ou uma sessão nova começar, o estado é recuperado a partir deles (ver <working_method>), não da conversa.
</docs_as_source_of_truth>
